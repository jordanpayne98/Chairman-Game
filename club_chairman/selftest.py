...
def run():
    ...
    if len(s['leagues']['divisions'])!=2 or len(s['career']['history'][0]['leagues']['movements'])!=6:raise RuntimeError('Missing divisions or promotion/relegation')
    ...
    if sum(len(h['fixtures']) for h in s['career']['history'])+len(s['fixtures'])!=254:raise RuntimeError('Incomplete league/cup fixture inventory')
