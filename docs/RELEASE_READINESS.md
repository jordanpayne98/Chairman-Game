...
## Current playable scope: Football and Development Update 0.7
...
| 9–11: people and knowledge | Approved 37-attribute catalogue, position-weighted overall, one-time potential sampling, weekly development and bounded youth potential reviews; dated uncertain estimates, hidden-data-safe filtering/comparison | Complete role/retraining models, mature/setback potential policy, staff capability effects, richer evidence-based scouting, staff knowledge and discovery |
...
| 22–24: interface | Executive shell, complete attribute profile groups, development/medical views, registration review, match statistics, department/player/help search, existing commercial/contract/career screens | Remaining departmental workflows, profile side panels, responsive text scaling and consistent validation/denial coverage |
| 25–26: data and saves | Snapshot tuning, JSON inside atomic SQLite, checksum verification, backups and migrations 1→2→3→4→5→6; original-engine completion of old in-progress matchdays | Pack import/conflict tooling, approved mod formats, full normalised/indexed event storage and world-scale performance |
...
| 37–39: system/UI QoL | Shortlist/notes/pins, estimates/cost comparisons, forecast, history memory, offer/registration drafts, unread distinction, global search and tooltips | Named lists/scenarios, advanced tables, safe bulk operations, notification policies, saved layouts, calendars, complete recovery browser and all QAT workflows |
...
1. Finish the remaining contract clauses and competition policies; implement full staff capabilities and autonomous AI finances/authority on the new player/registration foundation.
